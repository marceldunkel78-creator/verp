...
const CustomerModal = ({ customer, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        await api.put(`/customers/customers/${customer.id}/`, submitData);
      } else {
                    ...
                    <div className="p-4 bg-gray-50" style={{ height: '500px' }}>
                      {selectedAddress?.latitude && selectedAddress?.longitude ? (
                        <iframe
                          ...
                          marginHeight="0"
                          marginWidth="0"
                          src={`https://www.openstreetmap.org/export/embed.html?bbox=${parseFloat(selectedAddress.longitude)-0.01},${parseFloat(selectedAddress.latitude)-0.01},${parseFloat(selectedAddress.longitude)+0.01},${parseFloat(selectedAddress.latitude)+0.01}&layer=mapnik&marker=${selectedAddress.latitude},${selectedAddress.longitude}`}
                        ...
                        <div className="flex items-center justify-center h-full text-gray-500">
                          <div className="text-center">
                            ...
                            <p>Koordinaten eingeben, um Karte anzuzeigen</p>
                            <p className="text-xs mt-2">
                              Tipp: Koordinaten bei{' '}
                              <a
