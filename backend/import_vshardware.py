#!/usr/bin/env python
"""
Import VS-Hardware Products from CSV file
...
from manufacturing.models import VSHardware, VSHardwarePrice
...
def import_vshardware_products(csv_path):
    """Import VS-Hardware products from CSV"""
    ...
    print(f"Importing VS-Hardware Products from: {csv_path}")
    ...
    # Use csv.reader for proper CSV parsing (handles quoted fields with newlines)
    ...
    for line_num, row in enumerate(rows[1:], start=2):
        try:
            ...
            if not name:
                print(f"Line {line_num}: Skipped - missing name")
                skipped_count += 1
                continue
            ...
            existing = VSHardware.objects.filter(
                name__iexact=name
            ).first()
            
            if existing:
                print(f"Line {line_num}: Skipped - product '{name}' already exists ({existing.part_number})")
                skipped_count += 1
                continue
            
            # Create product (use kennung as model_designation)
            product = VSHardware.objects.create(
                name=name,
                model_designation=kennung,
            ...
            # Create price record
            VSHardwarePrice.objects.create(
                vs_hardware=product,
            ...
            print(f"Line {line_num}: ✓ Created {product.part_number} - {name[:50]}... (EK: {purchase_price}€, VK: {list_price}€)")
            imported_count += 1
...
if __name__ == '__main__':
    ...
    import_vshardware_products(csv_file)
