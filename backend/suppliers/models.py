...
class Asset(models.Model):
    """
    Anlagen - Instanzierte Waren die bestellt und verwaltet werden
    """
    ...
    product_group = models.ForeignKey(
        ...
        related_name='assets',
        verbose_name='Warengruppe',
        help_text='Automatisch auf "Anlage" gesetzt'
    )
    ...
    warranty_months = models.IntegerField(
        default=12,
        verbose_name='Garantiezeitraum (Monate)'
    )
    current_value = models.DecimalField(
        ...
        null=True,
        blank=True,
        verbose_name='Aktueller Restwert'
    )
    ...
    def __str__(self):
        return f"{self.visitron_part_number} - {self.name}" if self.visitron_part_number else self.name
    ...
    def _generate_visitron_part_number(self):
        """Generiert die nächste Visitron-Partnummer für Assets dieses Lieferanten"""
        if not self.supplier or not self.supplier.supplier_number:
            raise ValueError("Lieferant muss eine supplier_number haben")
        
        # Finde alle existierenden Asset-Partnummern für diesen Lieferanten
        # Format: XXX-AYYY (A für Asset)
        prefix = f"{self.supplier.supplier_number}-A"
        existing_numbers = Asset.objects.filter(
            visitron_part_number__startswith=prefix
        ).values_list('visitron_part_number', flat=True)
        
        # Extrahiere die Nummern nach dem "A"
        suffix_numbers = []
        for number in existing_numbers:
            parts = number.split('-A')
            if len(parts) == 2 and parts[1].isdigit():
                suffix_numbers.append(int(parts[1]))
