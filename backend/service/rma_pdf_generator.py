...
class RMADeliveryNoteDocTemplate(BaseDocTemplate):
    """
    Custom DocTemplate für RMA-Lieferscheine mit Header und Footer
    """
    ...
    def _add_header_footer(self, canvas, doc):
        ...
        if page_num > 1:
            ...
            header_text = f"{lang['page']} {page_num} - {lang['delivery_note']} {rma_return.return_number}"
...
def generate_rma_delivery_note_pdf(rma_return, language='de'):
    ...
    <b>{t['delivery_note_number']}:</b> {rma_return.return_number}<br/>
    <b>{t['rma_number']}:</b> {rma_case.rma_number}<br/>
    <b>{t['return_date']}:</b> {rma_return.return_date.strftime('%d.%m.%Y')}<br/>
    """
    meta_text += "</para>"
    ...
    if rma_return.shipping_carrier or rma_return.tracking_number:
        ...
        if rma_return.shipping_carrier:
            elements.append(Paragraph(f"{t['carrier']}: {rma_return.shipping_carrier}", style_small))
        if rma_return.tracking_number:
            elements.append(Paragraph(f"{t['tracking']}: {rma_return.tracking_number}", style_small))
        elements.append(Spacer(1, 0.5 * cm))

    # === NOTIZEN ===
    if rma_return.notes:
        elements.append(Paragraph(f"<b>{t['notes']}</b>", style_normal))
        elements.append(Paragraph(sanitize_for_pdf(rma_return.notes).replace('\n', '<br/>'), style_small))
        elements.append(Spacer(1, 0.5 * cm))
