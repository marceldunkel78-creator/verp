#!/usr/bin/env python
"""
Initialize new product categories for inventory management
"""
import os
import sys
import django
...
def initialize_new_categories():
    """Initialize the new categories: TIRF, FRAP-TIRF, Lichtleiter, Mikroskopadapter, HBO-XBO"""
    ...
    for code, name in new_categories:
        _, created = ProductCategory.objects.get_or_create(
            ...
            defaults={
                ...
                'is_active': True,
                'sort_order': 50,  # Insert after existing hardware categories
            }
        )
        if created:
            created_count += 1
